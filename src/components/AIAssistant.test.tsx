...
describe("AIAssistant Component", () => {
  ...
  // Helper to open chat
  const openChatAndSwitchLanguage = async (
    lang: "EN" | "FR" | "PID" = "EN"
  ) => {
    ...
    if (lang === "EN") {
      ...
      expectedInitialMessagePart =
        /Bonjour! Je suis votre assistant SiliconHub./i;
    } else {
      // PID
  ...
  it("renders closed initially, then opens and shows default language (English)", async () => {
    ...
    expect(
      screen.getByText(/Hello! I'm your SiliconHub assistant./i)
    ).toBeInTheDocument();
  ...
  it("changes UI elements to French when FR language is selected", async () => {
    ...
    expect(
      screen.getByText(/Bonjour! Je suis votre assistant SiliconHub./i)
    ).toBeInTheDocument();
  ...
  it("changes UI elements to Pidgin when PID language is selected", async () => {
    ...
    expect(
      screen.getByText(/Salut! Na me be ya SiliconHub assistant./i)
    ).toBeInTheDocument();
    expect(screen.getByText("🍽️ I wan chop")).toBeInTheDocument();
    expect(
      screen.getByPlaceholderText("Write ya message...")
    ).toBeInTheDocument();
  });
  ...
  it("bot asks for phone number on payment intent", async () => {
    ...
    await screen.findByText(
      /Okay, I can help with that. Please provide your phone number./i
    );
  });
  ...
  it("handles successful payment initiation", async () => {
    ...
    act(() => {
      vi.runAllTimers();
    }); // For setIsTyping and initial bot message

    // Bot acknowledges receiving number (paymentInitiationNoted)
    await screen.findByText(
      /Got it. I'll use the next message as your phone number for the payment./i
    );

    // Ensure fetch is called correctly
    expect(global.fetch).toHaveBeenCalledWith("/api/nkwa/pay/123456789", {
      method: "GET",
    });

    act(() => {
      vi.runAllTimers();
    }); // For fetch promise and subsequent bot message

    // Bot confirms successful payment
    await screen.findByText(
      /Your payment request has been initiated successfully./i
    );
  ...
  it("handles failed payment initiation (API error)", async () => {
    ...
    expect(global.fetch).toHaveBeenCalledWith("/api/nkwa/pay/987654321", {
      method: "GET",
    });
    act(() => {
      vi.runAllTimers();
    ...
    await screen.findByText(
      /Sorry, I couldn't initiate the payment. Please try again later./i
    );
  });
  ...
  it("handles invalid phone number provided", async () => {
    ...
    await screen.findByText(
      /The phone number provided seems invalid. Please provide a valid number./i
    );
