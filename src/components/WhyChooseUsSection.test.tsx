...
describe('WhyChooseUsSection', () => {
  ...
  it('renders the descriptive paragraph in English by default', () => {
    render(<WhyChooseUsSection />);
    expect(screen.getByText(/Your central hub for discovering and accessing the best digital services Cameroon has to offer/)).toBeInTheDocument();
  });
  ...
  it('switches content language to French when FR button is clicked', () => {
    ...
    expect(screen.getByText('Découvrez les Pépites Locales')).toBeInTheDocument();
  });
  ...
  it('switches content language to Pidgin when PID button is clicked', () => {
    ...
    expect(screen.getByText('Discover Local Correct Services')).toBeInTheDocument();
  });
});
